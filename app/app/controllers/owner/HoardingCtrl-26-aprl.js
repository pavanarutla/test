app.controller('OwnerHoardingCtrl', function ($scope, $mdDialog, $http,$mdSidenav,$location, AdminCampaignService, ProductService, toastr,MapService,config,CampaignService,$timeout) {
   ...
   // Load More
    ...
    //Shotlist the product
    $scope.shortlistSelected = function (id) {
        MapService.shortListProduct(id, JSON.parse(localStorage.OwnerloggedInUser).id).then(function (response) {
          $mdDialog.show(
            $mdDialog.alert()
    ...
    function getShortListedProducts() {
        MapService.getshortListProduct(JSON.parse(localStorage.OwnerloggedInUser).id).then(function (response) {
    ...
    $scope.viewProduct = function(product){
      ...
      $scope.deleteShortlisted = function (ev, productId) {
        ...
        MapService.deleteShortlistedProduct(JSON.parse(localStorage.OwnerloggedInUser).id, productId).then(function (response) {
          $mdDialog.show(
            $mdDialog.alert()
              .parent(angular.element(document.querySelector('body')))
              .clickOutsideToClose(true)
  ...
  $scope.saveCampaign = function($detail){
    ...
    if($scope.shortListedProducts.length > 0){
          ...
          if($scope.signup.$valid) {
            $scope.campaign.posted_by= JSON.parse(localStorage.OwnerloggedInUser).id;
