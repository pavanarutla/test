app.controller('OwnerHoardingCtrl', function ($scope, $mdDialog, $http,$mdSidenav,$location, AdminCampaignService, ProductService, toastr,MapService,config,CampaignService,$timeout,$rootScope) {
    ...
    */
    ...
    $rootScope.loadProductList = function(){
      var data = {pageNo:$scope.pageNo};    
      ProductService.ListofProductOwner(data).then(function(result){  
        $scope.productList = $scope.productList.concat(result);      
        $scope.productLength = result.length;
        if($scope.productLength > 0){
          $scope.pageNo += 1;
        }else{
  ...
  $scope.shareShortlistedProducts = function (shareShortlisted) {
    ...
    CampaignService.shareShortListedProducts(sendObj).then(function (result) {
      if(result.status == 1){
        toastr.success(result.message);
        $mdSidenav('shareProductOwner').close()
        $mdSidenav('shortlistAndSaveOwnerSidenav').close()
      }
      else{
        toastr.error(result.message);
  ...
  //console.log($rootScope.selectedProductID);
