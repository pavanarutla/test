  ...
  picker.directive('dateRangePicker', ['$compile', '$timeout', '$parse', 'dateRangePickerConfig', function($compile, $timeout, $parse, dateRangePickerConfig) {
    return {
      ...
      link: function($scope, element, attrs, modelCtrl) {
        ...
        _setStartDateForMultiple = _setDatePoint(function(m) {
          return _picker.setStartDateForMultiple(m);
        });
        ...
        _setStartDateForMultiple = _setDatePoint(function(m) {
          return _picker.setEndDateForMultiple(m);
        });
        ...
        modelCtrl.$formatters.push(function(objValue) {
          ...
          if (!opts.multipleDateRanges && opts.singleDatePicker && objValue) {
            return f(objValue);
          } else if (!opts.multipleDateRanges && objValue.startDate) {
            return [f(objValue.startDate), f(objValue.endDate)].join(opts.locale.separator);
          } else if(opts.multipleDateRanges){
            return '';
          }
        });
        modelCtrl.$render = function() {
          if (!opts.multipleDateRanges && modelCtrl.$modelValue && modelCtrl.$modelValue.startDate) {
            _setStartDate(modelCtrl.$modelValue.startDate);
            _setEndDate(modelCtrl.$modelValue.endDate);
          } else if(opts.multipleDateRanges) {
            $scope.model = _picker.selectedDateRanges;
            // _setStartDateForMultiple(modelCtrl.$modelValue);
            // _setEndDateForMultiple(modelCtrl.$modelValue);
          } else{
              _clear();
          }
        ...
        _init = function() {
          ...
          el.daterangepicker(angular.extend(opts, {
            ...
            return $scope.$apply(function() {
              if(opts.multipleDateRanges){
                return $scope.model = _picker.selectedDateRanges;
              }
              else{
          ...
          el.on('apply.daterangepicker', function(){
            $scope.$apply(function() {
              $scope.model = _picker.selectedDateRanges.map(function(sdr){
                return Object.assign({}, {
                  startDate : sdr.startDate.toISOString(),
                  endDate : sdr.endDate.toISOString()
                });
              });
              el.val("Selected ranges: " + _picker.selectedDateRanges.length);
            });
          });
          return results;
        };
        ...
        if(opts.multipleDateRanges){
          $scope.$watch('model', function(value){
            el.val("Selected ranges: " + _picker.selectedDateRanges.length);
            modelCtrl.$modelValue = _picker.selectedDateRanges.map(function(sdr){
              return Object.assign({}, {
                startDate : sdr.startDate.toISOString(),
                endDate : sdr.endDate.toISOString()
              });
